# EDAProject2ParticulateMatter
Air pollution measures of particulate matter from the National Emissions Inventory

My [Coursera-Exploratory-Data-Analysis](https://github.com/bbasberg/EDAProject2ParticulateMatter) Repo  
Please note that I used a separate R script file named **GetNEIDataFromEPA.R** to download the data and prep the subdirectories before running the six R scripts to generate the plots.  The R script files are named **plotx.R** where x is an integer from 1 to 6.  Similarly, the plots generated from the R scripts are named **plotn.png** where each n corresponds to the number in the name of the R script that generated it.

*I did end up just including the code from GetNEIDataFromEPA.R in each plot script rather than just sourcing it in each plot file for grading ease.*
